...
//View坐标转换为屏幕坐标，这里的屏幕坐标左上角原点
 ...
 { 
  	return vec4((vertex.xy / vertex.w) * (ubo.viewport/2.0f)+ubo.viewport/2.0f,vertex.z/vertex.w,1); 
 } 

 //屏幕坐标转NDC坐标，这里需要保持深度信息和原始是相同的,这里已经其次话为1
 vec4 ToNDCSpace(vec4 vertex)
 {
   return vec4((vertex.xy-(ubo.viewport/2.0f))/(ubo.viewport/2.0f),vertex.z,1);
 }
...
void main()
{
     gl_Position=gl_in[1].gl_Position.xy ;
     EmitVertex();
	 gl_Position=gl_in[1].gl_Position-vec4(0.01,0.01,gl_in[1].gl_Position.z,0);
	 EmitVertex();
	 gl_Position=gl_in[2].gl_Position ;
	 EmitVertex();
	 gl_Position=gl_in[2].gl_Position-vec4(0.01,0.01,gl_in[2].gl_Position.z,0);
	 EmitVertex();
...
//   
//	gl_Position = ToNDCSpace(vec4((p1.xy +  linestyle.Thickess * n1) , p1.z, 1.0) );
//	EmitVertex();
// }
// else { 
//   
...
//	EmitVertex();
//	
//	gl_Position = ToNDCSpace(vec4((p2.xy +  linestyle.Thickess * n1) , p2.z, 1.0 ));
//	EmitVertex();
//	
//	gl_Position = ToNDCSpace(vec4((p2.xy +  linestyle.Thickess * n2) , p2.z, 1.0 ));
//	EmitVertex();
//  }
//  else { 
//   
