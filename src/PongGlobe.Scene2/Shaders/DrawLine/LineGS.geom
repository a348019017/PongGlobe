...
//	 //这里存在一个精度的问题，假设p2.xy的精度已经很低了可能4-5位的精度，一旦乘法或除法之后，
//	 //vec2 v0=  normalize(p2.xy*vec2(640,360)/p2.w)-(p1.xy*vec2(640,360)/p1.w);
//	 //vec2 v1=normalize(p2.xy-p1.xy);	 	
//	 //if(ubo.viewport.xy!=vec2(640,360))return;	 
//     gl_Position= ToNDCSpace(toScreenSpace(p1));
//     EmitVertex();
//	 gl_Position=ToNDCSpace(toScreenSpace(p2));
//	 EmitVertex();	 	  
//	 EndPrimitive();   
...
//	EndPrimitive();
  ...
  if( dot(v0,n1) > 0 ) {      
	...
	gl_Position = ToNDCSpace(vec4((p1.xy +  width * n1) , 0, 1.0));
	EmitVertex();
 }
 else { 
  ...
  if( dot(v2,n1) < 0 ) {
	...
	gl_Position = ToNDCSpace(vec4(p2.xy +  width * n1 , 0, 1.0 ));
	EmitVertex();
	
	gl_Position = ToNDCSpace(vec4(p2.xy +  width * n2 ,0, 1.0 ));
	EmitVertex();
  }
  else { 
    vec4 tmp=   vec4(p2.xy -  width * n1 ,0 , 1.0);
